...
NHD_HR_lines <- readOGR(dsn = "NHD geodatabase.gdb", layer = "NHDFlowline") #flowlines clipped to basin
NHD_HR_VAA <- sf::st_read(dsn = "NHD geodatabase.gdb", layer = "NHDPlusFlowlineVAA")
NHD_HR_merged <- left_join(NHD_HR_lines, NHD_HR_VAA, by='NHDPlusID')
NHD_HR_merged.df <- filter(NHD_HR_merged, StreamOrde >= 3)
states <- st_read('US states shapefile', layer='states')
...
tmap_save(mapfin, '~Figures\\Fig1_map.jpg') 